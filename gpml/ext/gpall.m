function [varargout] = gpall(hyp, inf, mean, cov, lik, x, y, xs, ys)
...
cstr = cov{1}; cstr;if isa(cstr,'function_handle'), cstr = func2str(cstr); end
...
try                                                  % call the inference method
  ...
  if nargin>7   % compute marginal likelihood and its derivatives only if needed
    if isstruct(y)
      ...
      nlZs = zeros(1, numel(y)); dnlZs = cell(1, numel(y));
      if nargout<=1
          for i=1:numel(y)
              [post, nlZs(i)] = feval(inf{:}, hyp, mean, cov, lik, x{i}, y{i});              
          end
          nlZ = sum(nlZs);
          dnlZ = [];          
      else
          for i=1:numel(y)
              [post, nlZs(i), dnlZs{i}] = feval(inf{:}, hyp, mean, cov, lik, x{i}, y{i});
          end
          nlZ = sum(nlZs); 
          for i=1:numel(dnlZs)
...
if nargin==7                                     % if no test cases are provided
  ...
  error('currently not intended for this use, only for training');
  if isinf(post.sW)
    ...
    while nact<ns               % process minibatches of test cases to save memory
      ...
      if nargin<9
        [lp(id) ymu(id) ys2(id)] = feval(lik{:}, hyp.lik, [], fmu(id), fs2(id));
      else
        [lp(id) ymu(id) ys2(id)] = feval(lik{:}, hyp.lik, ys(id), fmu(id), fs2(id));
      end
